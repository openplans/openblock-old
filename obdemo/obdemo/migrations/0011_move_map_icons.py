...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for schema in orm['db.schema'].objects.all():
            if (schema.map_icon_url or '').startswith('/map_icons'):
                schema.map_icon_url = schema.map_icon_url.lstrip('/')
                schema.save()
    ...
    def backwards(self, orm):
        ...
        for schema in orm['db.schema'].objects.all():
            if (schema.map_icon_url or '').startswith('map_icons'):
                schema.map_icon_url = '/' + schema.map_icon_url
                schema.save()
